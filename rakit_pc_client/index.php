    ...
    <div id="wrapper">
        ...
        <ul class="navbar-nav bg-gradient-info sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <a class="sidebar-brand d-flex align-items-center justify-content-center klik_menu" id="#dashboard" href="#">
                ...
                <i class="fas fa-tools"></i>RAKIT PC</div><i class="fas fa-desktop"></i>
            </a>
            ...
            <li class="nav-item">
                <a class="nav-link klik_menu" id="pembeli" href="#">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.klik_menu').on('click', function() {
                ...
                if (menu == "dashboard") {
                    $('.val').load('dashboard.php');
                } else if (menu == "pembeli") {
                    $('.val').load('pembeli.php');
                    loadTablepembeli();
                } else if (menu == "Jadwal") {
                    $('.val').load('record.php');
                    loadTableJadwal();
            ...
            $('.val').on('click', '.addUst', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "http://localhost/rakitan_pc/Pembeli",
                    data: {
                        nama_pembeli: nama,
                        no_hp_pembeli: nohp,
            ...
            $('.val').on('click', '.editUst', function() {
                ...
                $.ajax({
                    type: "GET",
                    url: `http://localhost/rakitan_pc/Pembeli?id_pembeli=${id}`,
                    dataType: "JSON",
                    headers: {
                        'API-TOKEN': "6789"
                    },
                    success: function(response) {
                        response.data.forEach(element => {
                            $('[name="id"]').val(element.id_pembeli);
                            $('[name="updateNama"]').val(element.nama_pembeli);
                            $('[name="updateNoHp"]').val(element.no_hp_pembeli);
                            $('[name="updateAlamat"]').val(element.alamat_pembeli);
                        });
                    }
                });
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        response.data.forEach(element => {
                            ...
                            html += `<tr>
                                        <th scope="row">${i}</th>
                                        <td>${element.id_pembeli}</td>
                                        <td>${element.nama_pembeli}</td>
                                        <td>${element.alamat_pembeli}</td>
                                        <td>${element.no_hp_pembeli}</td>
                                        <td>
                                            <ul class="list-inline m-0">
                                                <li class="list-inline-item">
                                                    <button key="${element.id_pembeli}" class="btn btn-success btn-sm rounded-0 editUst" type="button" data-toggle="modal" data-target="#updateModalCenterUst"><i class="fa fa-edit"></i></button>
                                                </li>
                                                <li class="list-inline-item">
                                                    <button key="${element.id_pembeli}" class="btn btn-danger btn-sm rounded-0 deleteUst" type="button"><i class="fa fa-trash"></i></button>
                                                </li>
                                            </ul>
                                        </td>
                                    </tr>`;
            ...
            $('.val').on('click', '.create', function() {
                $.ajax({
                    ...
                    success: function(response) {
                        var html = "<option>Pilih pembeli</option>`";
                        response.data.forEach(element => {
                            html += `<option value ="${element.id_pembeli}">${element.nama_pembeli}</option>`;
                        });
                        $('.inputUst').html(html);
                    }
                });
            ...
            $('.val').on('click', '.addMsj', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "http://localhost/rakitan_pc/jadwal",
                    data: {
                        pembeli_id: id,
                        tanggal: tanggal,
                        waktu_mulai: mulai,
            ...
            $('.val').on('click', '.editMsj', function() {
                ...
                $.ajax({
                    type: "GET",
                    url: `http://localhost/rakitan_pc/jadwal?id_jadwal=${id}`,
                    dataType: "JSON",
                    headers: {
                        'API-TOKEN': "6789"
                    },
                    success: function(response) {
                        response.data.forEach(element => {
                            ...
                            $('[name="update_id_ust"]').val(element.pembeli_id);
